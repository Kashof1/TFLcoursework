...
from core.tfl import (
    get_tflstation,
    get_crowdingdata,
    get_disruptionstatus,
    get_statusseverity,
)
...
class tfl_dataCollector:
    def __init__(
        self, line, station, station_api, crowding_api, disruption_api, status_api
    ) -> None:
        self.line = line
    ...
    def collector(self):
        ...
        if colName not in existingCols:
            currentCol = db.create_collection(
                colName,
                timeseries={
                    "timeField": "time",
                    "metaField": "meta",
                ...
                for currentTrainid in list(currentTrains):
                    if currentTrainid == "TrainID":  # skipping header
                        ...
                        if (
                            ...
                            if not currentCol.count_documents(
                                {"meta.predictedTime": predictedTime}
                            ):
